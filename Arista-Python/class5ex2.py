...
def main():
    ...
    # Argument parsing
    parser = argparse.ArgumentParser(description="Idempotent addition/removal of
 VLAN to Arista switch")
    parser.add_argument("vlan_id", help="VLAN number to create or remove", actio
n="store", type=int)
    parser.add_argument("--name", help="Specify VLAN name", action="store", dest
="vlan_name", type=str)
    parser.add_argument("--remove", help="Remove the given VLAN ID", action="sto
re_true")
...
'''
>>> command_str = 'show vlan id 500'
>>> commands = [command_str]
>>> response = eapi_conn.config(commands)
>>> pprint.pprint(response)
[{u'sourceDetail': u'',
  u'vlans': {u'500': {u'dynamic': False,
                      u'interfaces': {},
                      u'name': u'orange',
                      u'status': u'active'}}}]
>>> type(response)
...
{u'sourceDetail': u'',
 u'vlans': {u'500': {u'dynamic': False,
                     ...
                     u'name': u'orange',
                     u'status': u'active'}}}
>>> test = response[0]['vlans']
>>> test
{u'500': {u'status': u'active', u'interfaces': {}, u'dynamic': False, u'name': u
'orange'}}
>>> pprint.pprint(test)
{u'500': {u'dynamic': False,
          u'interfaces': {},
          u'name': u'orange',
          u'status': u'active'}}
>>> test = response[0]['vlans']['500']
>>> pprint.pprint(test)
{u'dynamic': False,
 u'interfaces': {},
 u'name': u'orange',
 u'status': u'active'}
>>> test = response[0]['vlans']['500']['name']
>>> pprint.pprint(test)
u'orange'
